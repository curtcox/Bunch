 ...
 */
public class WeightedObjectiveFunctionCalculator3 implements ObjectiveFunctionCalculator {
 ...
 * Class constructor
 */
public WeightedObjectiveFunctionCalculator3() { }
...
calculate()
{
  ...
  double []intraTally = new double[clusterMatrix_d.length];
  double []interTally = new double[clusterMatrix_d.length];

  for (int i=0; i<clusterMatrix_d.length; ++i)
	intraTally[i] = interTally[i] = 0.0;


  for (int i=0; i<clusterMatrix_d.length; ++i) {
    if (clusterMatrix_d[i][0] > 0) {
      int[] clust = clusterMatrix_d[i];
      objTalley += calculateIntradependenciesValue(clust, i, intraTally, interTally);
      k++;
    }
  }
...
public
double
calculateIntradependenciesValue(int[] c, int numCluster, double[] intraTally, double[] interTally)
{
  double intradep=0.0;
...
//System.out.println("Edge weight = " + w[j]);
          intradep += w[j];
          intraEdges++;
		  intraTally[numCluster]+=w[j];
        }
        else
        {
          exitEdges += w[j];
          interEdges++;
		  interTally[numCluster] += (0.5 * w[j]);
		  interTally[nodes_x[c2[j]].cluster] += (0.5 * w[j]);
        }
      }
    }
  }
